        ...
        <div class="row">
          <div class="col-xs-12 col-md-8 px20">
            <slot name="placeOrderButton">
              <paypal-button
                v-if="isPayPal && !$v.orderReview.$invalid"/>
              <button-full
                @click.native="placeOrder"
                data-testid="orderReviewSubmit"
                class="place-order-btn"
                v-if="!isPayPal"
                :disabled="$v.orderReview.$invalid || isPayPal"
              >
                {{ $t('Place the order') }}
              </button-full>
            </slot>
...
export default {
  ...
  computed: {
    isPayPal: function () {
      return this.$store.state.checkout.paymentDetails.paymentMethod === config.paypal.methodId
    }
  },
  methods: {
    onSuccess () {
