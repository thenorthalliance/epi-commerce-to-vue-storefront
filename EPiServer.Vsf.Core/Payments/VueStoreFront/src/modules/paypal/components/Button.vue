...
export default {
  ...
  methods: {
    configurePaypal () {
      let defaultStyle = { 'size': 'responsive' }
    ...
    getCartData () {
      return {
        cartId: store.state.cart.cartServerToken,
        transaction: {
          amount: {
            total: this.grandTotal, currency: this.currency
          }
        }
      }
    },
    createPayment (data, actions) {
      return store.dispatch('paypal/create', this.getCartData())
    },
